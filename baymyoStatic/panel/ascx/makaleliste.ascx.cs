...
namespace baymyoStatic.panel.ascx
{
    public partial class makaleliste : System.Web.UI.UserControl
    {
        ...
        {
            using (BAYMYO.UI.Web.DataPagers data = new BAYMYO.UI.Web.DataPagers(dataGrid1, "makale", "id desc", "1=1"))
            {
            ...
            try
            {
                if (ddlIslemler.SelectedIndex > 0 & Core.IsUserAdmin)
                {
                    ...
                    if (ddlIslemler.SelectedIndex == 1 || ddlIslemler.SelectedIndex == 2)
                    {
                        foreach (GridViewRow item in dataGrid1.Rows)
                            if (((CheckBox)item.Cells[0].FindControl("chkSelected")).Checked)
                                Core.Update("makale", "yoneticionay", BAYMYO.UI.Converts.NullToInt64(dataGrid1.DataKeys[item.RowIndex][0]), chkState);
                    ...
                    else if (ddlIslemler.SelectedIndex == 3 || ddlIslemler.SelectedIndex == 4)
                    {
                        foreach (GridViewRow item in dataGrid1.Rows)
                            if (((CheckBox)item.Cells[0].FindControl("chkSelected")).Checked)
                                Core.Update("makale", "aktif", BAYMYO.UI.Converts.NullToInt64(dataGrid1.DataKeys[item.RowIndex][0]), chkState);
                        jSonData.CreateData("makaleler");
                        GetDataPaging();
                    }
                }
